...
async def hyper_upload_file(client, file_path):
    """Upload a file in chunks with parallel connections."""
        ...
        async with semaphore:
            if is_big:
                ...
                await client(functions.upload.SaveFilePartRequest(
                    file_id=file_id, file_part=part_index, bytes=chunk_data
                ))
...
async def upload_single_file_logic(client, original_file, caption_prefix):
    """Upload a single file, splitting if >1.9GB."""
    ...
    for index, current_file in enumerate(files_to_upload, start=1):
        ...
        if is_big:
            media = InputMediaUploadedDocument(
                ...
                mime_type='application/octet-stream', attributes=attributes
            )
        else:
            media = InputMediaUploadedDocument(
        ...
        await client(functions.messages.SendMediaRequest(
            peer=peer, media=media, message=caption_text,
...
async def process_downloaded_item(client, item_path, is_folder, caption_prefix):
    """Process a downloaded file or folder recursively."""
    if is_folder:
        ...
        for idx, file_path in enumerate(all_files, start=1):
            # Move file to current dir (where splitting logic expects it)
            shutil.move(file_path, os.path.basename(file_path))
            await upload_single_file_logic(client, os.path.basename(file_path),
                                           f"{caption_prefix}\nFile {idx}/{total}")
